...
complement_scoremat(const std::vector<std::vector<float>> coltypes,
  ...
  for (size_t i = 0; i < coltypes.size(); ++i)
    for (size_t j = 0; j < coltypes[i].size(); j++)
      max_score = std::max(max_score, scoremat[i][j]);
...
struct dme_zoops_lextree {
  ...
  void
  build(const std::string &sequence, const size_t motif_width);

  void
  get_score(
    const std::vector<std::vector<std::vector<float>>> &refined_score_matrix,
    const std::vector<size_t> &path, size_t depth, float current_score,
...
dme_zoops_lextree::build(const std::string &sequence,
                         const size_t motif_width) {
  allocate_child_ptrs();
  const size_t n_substrings = std::size(sequence) - motif_width + 1;
...
void
dme_zoops_workspace::enumeration(const size_t depth,
                                 const float surplus_information) {
...
dme_zoops_workspace::dme_zoops_workspace(
  ...
  const std::vector<std::string> &background, const size_t width,
